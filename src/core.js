...
function initialWorkBook() {
    luckysheetHandler(); //Overall dom initialization 整体dom初始化
    initialFilterHandler(); //Filter initialization 过滤器初始化
    initialMatrixOperation(); //Right click matrix initialization 右键矩阵初始化
    initialSheetBar(); //bottom sheet bar initialization 底部sheet栏初始化
    formulaBarInitial(); //top formula bar initialization 顶部formula bar初始化
    rowColumnOperationInitial(); //row and coloumn operate initialization 行列操作初始化
    keyboardInitial(); //Keyboard operate initialization 键盘操作初始化
    orderByInitial(); //menu bar orderby function initialization 菜单栏orderby函数初始化
    zoomInitial(); //zoom method initialization 缩放方法初始化
    // printInitial(); //print initialization 打印初始化
    initListener();
}
